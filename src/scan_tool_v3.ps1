...
if ($Host.Name -match 'ISE') {
    Write-Host 'NetScan v3 는 PowerShell ISE 를 지원하지 않습니다.' -ForegroundColor Red
...
$script:ConfigPath = Join-Path $script:ScriptDir 'scan_tool.config.json'
...
function Show-Dashboard {
    ...
    $header = ' NetScan v3   {0} ({1})   {2} ~ {3} (/{4}, {5} hosts)   엔진: {6}' -f `
...
function Show-Settings {
    param($AppState)
    $width = [Console]::BufferWidth
    Write-Line -Row 0 -Text (' NetScan v3 ─ 설정                    저장 위치: scan_tool.config.json') -Color Cyan
    Write-Line -Row 1 -Text ('─' * ($width - 1)) -Color DarkGray
...
function Show-Detail {
    ...
    Write-Line -Row 0 -Text ' NetScan v3 ─ 호스트 상세                                        [Esc] 목록으로' -Color Cyan
    Write-Line -Row 1 -Text ('─' * ($width - 1)) -Color DarkGray
    if ($null -eq $h) {
...
try {
    ...
    [Console]::CursorVisible = $true
    Clear-Host
    Write-Host 'NetScan v3 를 종료했습니다.' -ForegroundColor Cyan
